  ...
  def score
    ...
    10.times do
      if is_strike?(frame_index)
        calc_score += 10 + strike_bonus(frame_index)
        frame_index += 1
      elsif is_spare?(frame_index)
        calc_score += 10 + spare_bonus(frame_index)
        frame_index += 2
      else
        calc_score += frame_score(frame_index)
        frame_index += 2
      end
    end
    calc_score
...
RSpec.describe "BowlingGame" do
  ...
  context "with a spare" do
    it "adds the next roll's value to the spare frame's score" do
      roll_spare
      game.roll(3)
      roll_many(17, 0)
  ...
  context "with a strike" do
    it "adds the next frame's score to the strike frame's score" do
      roll_strike
      game.roll(3)
      game.roll(4)
      roll_many(16, 0)
